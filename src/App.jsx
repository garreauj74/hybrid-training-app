...
export default function App() {
  ...
  // Active phase -- 1 or 2
  const [activePhase, setActivePhase] = useLocalStorage('active-phase', 1);

  // Phase-aware data helpers
  const currentWeekNotes = activePhase === 2 ? PHASE2_WEEK_NOTES : WEEK_NOTES;
  const currentDays = activePhase === 2 ? PHASE2_DAYS : DAYS;
  const currentGetWeekSets = activePhase === 2 ? getPhase2WeekSets : getWeekSets;
  ...
  const sharedProps = { activeWeek, setActiveWeek, activeDay, setActiveDay, isMobile, startDate, setStartDate, syncFromDate, activePhase, setActivePhase, currentWeekNotes, currentDays, currentGetWeekSets };
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 980, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px' }}>
          ...
          <div style={{ fontSize: isMobile ? 11 : 12, color: '#666', fontFamily: 'monospace' }}>
            Phase {activePhase} · {activePhase === 1 ? 'Foundation' : 'Development'}
          </div>
        </div>
      </div>
